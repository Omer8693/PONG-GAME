...
def train_reinforce(complex_mode=False):
    ...
    log_dir = os.path.join(Constants.tensorboard_log, f"{Constants.REINFORCE_sub_folder}_{'complex' if complex_mode else 'simple'}")
    ...
    try:
        ...
        plt.title(f"REINFORCE Training ({mode_str} mode)")
        plt.grid(True)
        plt.savefig(f"reinforce_training_curve_{mode_str}.png")
        plt.close()
...
if __name__ == "__main__":
    train_reinforce(complex_mode=False)
    train_reinforce(complex_mode=True)
